        ...
        this.doFilter = function(filterParams) {
            ...
            if (filterDOM === null) {
                if (voview.filter_xsl) {
                    ...
                    var filterGet = meFilter.makeGetXml({ fileUrl: "@XSL_PATH@filter.xsl",
            ...
            if (preProcDOM === null) {
                if (voview.preproc_xsl) {
                    ...
                    var preprocGet = meFilter.makeGetXml({ fileUrl: "@XSL_PATH@preProcess.xsl",
...
voview.prototype.makeRenderer = function(rendererParams) {
    ...
    function Renderer(_filterObject, objectName, widgetIDprefix) {
            ...
            if (rowSelection)
            {
              for (var jrow = 0; jrow < rows.length; jrow++)
              {
                if (rows[jrow].parentNode.tagName === "TBODY")
                {
                  ...
                  rows[jrow].insertBefore(newTD, rows[jrow].firstChild);
                ...
                if ((rows[jrow].parentNode.tagName === "THEAD") || (rows[jrow].parentNode.tagName === "TFOOT"))
                {
                  ...
                  else if (rows[jrow].parentNode.tagName === "TFOOT")
                  {
                    ...
                    rows[jrow].insertBefore(newTH, rows[jrow].firstChild);
                  }
                  else
                  {
                    if (allRowsSelected)
                    {
                      ...
                      var onClickAction = objectName + ".renderObject.clearRowSelection(); return false;";

                      newInput.setAttribute("type", "checkbox");
                      newInput.setAttribute("name", "selectAllCheckbox");
                      newInput.setAttribute("onclick", onClickAction);
                    ...
                    else
                    {
                      var newTH = document.createElement("th");
                      var newInput = document.createElement("input");
                      var onClickAction = objectName + ".renderObject.selectAllRows(); return false;";
        ...
        this.newPage = function(pageNumber) {
          ...
          {
            alert("Not a number: " + pageNumber);
          }
          else
          {
            var endElement =
                meRenderer.selectSingleNode(filteredDOM,
                                            "//*[@ID='VOV:PageEnd']");
            ...
            var rowCountElement =
                meRenderer.selectSingleNode(filteredDOM,
                                            "//*[@ID='VOV:FilterCount']");
            var totalRowCount = Number(rowCountElement.getAttribute("value"));
            var lastPage = Math.ceil(totalRowCount / pageLength);

            if (lastPage == 0)
            {
              // Do nothing as the filter set is empty.
            }
            else if ((pageNumber > lastPage) || (pageNumber < 1))
            {
              alert("Value out of range.  Enter page number between 1 and "
                    + lastPage + ".");
            }
            else
            {
            ...
            for ( var j = 0; j < form.elements.length; j++) {
                ...
                if (el.tagName === "INPUT" && el.type === "text" && !el.className.match("defaultComment")) {
                    ...
                    if (constraint) {
                        ...
                        isCharType = isCharType.toLowerCase() !== "false";
